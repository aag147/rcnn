# -*- coding: utf-8 -*-
"""
Created on Mon Apr 30 17:45:09 2018
...
class ProposalTarget(keras.layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        target_human_bounding_boxes, \
        target_object_bounding_boxes, \
        target_classes, \
        output_proposal_human_bounding_boxes, \
        output_proposal_object_bounding_boxes \
             = inputs
...
#        )
        ...
        output_proposal_human_bounding_boxes = output_proposal_human_bounding_boxes[batch_index, :, :]
        output_proposal_object_bounding_boxes = output_proposal_object_bounding_boxes[batch_index, :, :]
        target_human_bounding_boxes = target_human_bounding_boxes[batch_index, :, :]
        target_object_bounding_boxes = target_object_bounding_boxes[batch_index, :, :]
        target_classes = target_classes[batch_index, :, :]

        # TODO: Fix hack
#        condition = keras.backend.not_equal(keras.backend.sum(target_human_bounding_boxes), 0)
...
#                                              lambda: test(output_proposal_bounding_boxes, target_bounding_boxes, target_categories))
        ...
        sample_outputs = self.sample(output_proposal_human_bounding_boxes, output_proposal_object_bounding_boxes, \
                                     target_human_bounding_boxes, target_object_bounding_boxes, target_classes)

        output_proposal_human_bounding_boxes = keras.backend.expand_dims(sample_outputs[0], 0)
        output_proposal_object_bounding_boxes = keras.backend.expand_dims(sample_outputs[1], 0)
        target_classes = keras.backend.expand_dims(sample_outputs[2], 0)
        bounding_human_box_targets = keras.backend.expand_dims(sample_outputs[3], 0)
        bounding_object_box_targets = keras.backend.expand_dims(sample_outputs[4], 0)

        return [bounding_human_box_targets, bounding_object_box_targets, target_classes, \
                output_proposal_human_bounding_boxes, output_proposal_object_bounding_boxes]
    ...
    def sample(self, human_proposals, object_proposals, true_human_bounding_boxes, true_object_bounding_boxes, true_classes):
        """
        ...
        """
        number_of_classes = keras.backend.shape(true_classes)[1]

        true_classes = keras.backend.argmax(true_classes, axis=1)

        intersection_over_union = rcnn.backend.intersection_over_union(human_proposals, object_proposals, true_human_bounding_boxes, true_object_bounding_boxes)

        gt_assignment = keras.backend.argmax(intersection_over_union, axis=2)

        maximum_intersection_over_union = keras.backend.max(intersection_over_union, axis=2)
        ...
        all_labels = keras.backend.gather(true_classes, gt_assignment)
