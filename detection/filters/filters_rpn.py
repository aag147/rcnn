...
### PROCESS TARGETS ###
#######################
def createTargetsNope(imageMeta, imageDims, cfg):
    #in: imageMeta
    #out: non-reduced targets
    ...
    best_anchor_for_gtbox = -1*np.ones((num_bboxes, 3)).astype(int)
...
#    draw.drawHOI(image, gta[0,:], gta[0,:])
    ...
    shift_x = np.arange(0, width) * rpn_stride
    shift_y = np.arange(0, height) * rpn_stride
    
    shift_x, shift_y = np.meshgrid(shift_y, shift_x)
    shifts = np.vstack((np.expand_dims(shift_y, axis=0), np.expand_dims(shift_x, axis=0),
                        np.expand_dims(shift_y, axis=0), np.expand_dims(shift_x, axis=0))).transpose()    
    A = cfg.nb_anchors
    
    (K1,K2,_) = shifts.shape
    print(A, K1,K2, shifts.shape)
    
    all_anchors = (anchors.reshape((1, A, 4)) +
                   shifts.reshape((1, K1*K2, 4)).transpose((1, 0, 2)))
    all_anchors = all_anchors.reshape((K1, K2, A, 4))
...
#    
#    # min to center
#    all_anchors[:,:,:,0] += 0.5 * (all_anchors[:,:,:,2])
#    all_anchors[:,:,:,1] += 0.5 * (all_anchors[:,:,:,1])
    ...
    for ix in range(output_width):      
        for jy in range(output_height):
            anchors_ij = all_anchors[jy,ix]
            
            for anc_idx, anc in enumerate(anchors_ij):
                xmin_anc, ymin_anc, xmax_anc, ymax_anc =  anc
        ...
        #                            print('bb',bxmin, bymin, bxmin + bw, bymin + bh)
                    ...
                    if curr_iou > best_iou_for_gtbox[gtidx]:
                        best_anchor_for_gtbox[gtidx] = [jy, ix, anc_idx]
...
#            print('no anchors', idx, gta[idx])
            ...
            anc_idx = best_anchor_for_gtbox[idx,2]
            y_is_box_valid[
...
#            h_anc = anchor_sizes[anchor_size_idx] * anchor_ratios[anchor_ratio_idx][1]
            ...
            print(anchor_ratios[anchor_ratio_idx], anchor_sizes[anchor_size_idx], w, h, w_anc, h_anc)
