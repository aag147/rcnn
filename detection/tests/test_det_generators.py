...
#    draw.drawAnchors(img, boxes_nms, cfg)
    ...
    h_bboxes, o_bboxes, hoi_labels, patterns, final_vals = filters_hoi.prepareTargets(boxes_nms, imageMeta, imageDims, cfg, class_mapping)
    h_bboxes_norm, o_bboxes_norm = filters_hoi.prepareInputs(h_bboxes, o_bboxes, imageDims)

    hoiidx = 0
    for hoiidx in range(0):
        draw.drawHoIComplete((X[0]+1.0)/2.0, h_bboxes[hoiidx,:], o_bboxes[hoiidx,:], patterns[hoiidx,::], hoi_labels[hoiidx,:], labels)
    break
