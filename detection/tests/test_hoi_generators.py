...
for i in range(0):
    ...
    rels = imageMeta['rels']
    if rels.shape[0]>1 and rels[0,1] > rels[1,1]:
        print(imageMeta['rels'])
        break
    else:
        continue
...
#        draw.drawHoIComplete((X[0]+1.0)/2.0, h_bboxes[hoiidx,:], o_bboxes[hoiidx,:], ip[0,hoiidx,::], target_labels[0,hoiidx,:], labels, cfg)
    draw.drawPositiveHoIs((X[0]+1.0)/2.0, h_bboxes, o_bboxes, allhois[0,::], labels, imageMeta, imageDims, cfg)